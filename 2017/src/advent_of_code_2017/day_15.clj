...
;; before comparing them (using the same comparison method as
...
(defn compare-generators-part-2 [{:keys [a-initial b-initial]} max-comparisons]
  ;; start with a call to rest so that we don't count the initial
  ;; value
  (loop [a (drop-while #(not (divisible-by 4 %)) (generator-a a-initial))
         b (drop-while #(not (divisible-by 8 %)) (generator-b b-initial))
         tally 0
         comparisons 0]
    (if (and (> max-comparisons comparisons))
      (let [this-a (next-16-bits a)
            this-b (next-16-bits b)
            rest-a (->> a
                        (drop 1)
                        (drop-while #(not (divisible-by 4 %))))
            rest-b (->> b
                        (drop 1)
                        (drop-while #(not (divisible-by 8 %))))
            next-tally (if (= this-a this-b)
                         (inc tally)
                         tally)]
        (recur rest-a rest-b next-tally (inc comparisons)))
