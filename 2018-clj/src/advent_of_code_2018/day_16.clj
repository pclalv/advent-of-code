...
;; inputs (named A and B), and an output (named C), in that order. The
;; opcode specifies the behavior of the instruction and how the inputs
...
;; B, storing the result in register C, and the instruction addi 0 7 3
;; is encountered, it would add 7 to the value contained by register 0
;; and store the sum in register 3, never modifying registers 0, 1, or
;; 2 in the process.
...
;; 1. After the instruction is executed, register 2's value becomes 2.
...
;; 1. Opcode 9 could be mulr: register 2 (which has a value of 1)
;;    times register 1 (which has a value of 2) produces 2, which
;;    matches the value stored in the output register, register 2.

;; 2. Opcode 9 could be addi: register 2 (which has a value of 1) plus
;;    value 1 produces 2, which matches the value stored in the output
;;    register, register 2.

;; 3. Opcode 9 could be seti: value 2 matches the value stored in the
;;    output register, register 2; the number given for B is
;;    irrelevant.

;; None of the other opcodes produce the result captured in the
;; sample. Because of this, the sample above behaves like three
;; opcodes.
...
(defn riffled? [[head-all & tail-all :as all]
                [head-half1 & tail-half1 :as half1]
                [head-half2 & tail-half2 :as half2]]
  (cond (nil? head-all) true
        (= head-all head-half1) (recur tail-all tail-half1 half2)
        (= head-all head-half2) (recur tail-all half1 tail-half2)
        :else false))
