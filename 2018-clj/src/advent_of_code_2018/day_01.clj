...
(defn input-file-lines [input-file]
  (-> (str "resources/day_01/" input-file)
      (slurp)
      (clojure.string/split #"\n")))

(defn part1 [input-file]
  (let [frequency-changes (->> input-file
                               (input-file-lines)
...
(defn find-repeated-frequency
  ([frequency-changes]
   (find-repeated-frequency frequency-changes {:metafrequencies {} :current-frequency 0}))
  ([[frequency-change & frequency-changes] {metafrequencies' :metafrequencies current-frequency' :current-frequency}]
   (let [current-frequency (+ current-frequency' frequency-change)
         metafrequency (get metafrequencies' current-frequency 0)]
     (if (= metafrequency 1)
       current-frequency
       (let [metafrequencies (assoc metafrequencies'
                                    current-frequency
                                    (inc metafrequency))]
         (recur frequency-changes {:metafrequencies metafrequencies :current-frequency current-frequency}))))))
...
(defn part2 [input-file]
  (let [frequency-changes (->> input-file
                               ...
                               (map #(Integer/parseInt %))
                               (cycle))]
    (find-repeated-frequency frequency-changes)))
