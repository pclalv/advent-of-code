...
(defn no-repetitions? [words]
  (= (count (set words))
     (count words)))

(defn count-valid-passphrases
  ([passphrase-file]
   (count-valid-passphrases no-repetitions? passphrase-file))
  ([predicate passphrase-file]
   (with-open [rdr (clojure.java.io/reader passphrase-file)]
     (let [passphrases (line-seq rdr)
           passphrases-words (map #(str/split % #" ") passphrases)]
       (count (filter predicate passphrases-words))))))
...
(defn no-anagrams? [passphrase-words]
  (no-repetitions? (map sort passphrase-words)))

(comment
  ;; part 1 soln
  (count-valid-passphrases "resources/day-4/input")
  ;; part 2 soln
  (count-valid-passphrases no-anagrams? "resources/day-4/input"))
